...
logging.basicConfig("template_prepare.log", level=logging.WARNING, format=LOG_FORMAT, datafmt=DATE_FORMAT)
...
class RxnTemplatePrepare:
        ...
        with tqdm(total=len(lines)) as pbar:
            for n, line in enumerate(lines):
                if n == 0:
                    ...
                    cls._init_result_file(config.rid_with_rxn_template_tsv_file_path,
                                          "rid\trxn_centralized_template\trxn_extended_template")
                else:
                    rid = line.split('\t')[rid_idx]
                    rxn_smi = line.split('\t')[rxn_smi_idx]
                    reaction_centralized_template = cls._get_rxn_template(rxn_smi, RxnTemplateType.CENTRALIZED)
                    reaction_extended_template = cls._get_rxn_template(rxn_smi, RxnTemplateType.EXTENDED)
                    if reaction_centralized_template is not None:
                        num_rct += 1
                    if reaction_extended_template is not None:
                        num_ret += 1
                    results.append(f"{rid}\t{reaction_centralized_template}\t{reaction_extended_template}")
                    if len(results) % 1000 == 0:
                        cls._add_lines(config.rid_with_rxn_template_tsv_file_path, results)
                        results = []
                pbar.update(1)
    ...
    def organize_rxn_template(cls, config: Config):
        rid_rxn_template_df = pd.read_csv(config.rid_with_rxn_template_tsv_file_path, sep='\t', encoding='utf-8')
    ...
    def link_rid_and_tid(cls, config: Config):
        ...
        rid_with_temp_df = pd.read_csv(config.rid_with_rxn_template_tsv_file_path, sep='\t', encoding='utf-8')
        idx = 0
        cls._init_result_file(config.rid_with_tid_tsv_file_path, "rid\tc_tid\te_tid\tc_num\te_num")
        res_lines = []
        with tqdm(total=len(rid_with_temp_df), desc="link rid and tid")as pbar:
            for i, row in rid_with_temp_df.iterrows():
                ...
                if len(res_lines) == 10000:
                    cls._add_lines(config.rid_with_tid_tsv_file_path, res_lines)
                    res_lines = []
                idx += 1
                pbar.update(1)
        if len(res_lines) > 0:
